...
public class Driver extends Thread {
	...
	public static String USER_NAME = "IBRAHIM";
	static DatagramSocket mSocket;
	static ArrayList<ChatFrame> IBRAChats = new ArrayList<ChatFrame>();
	...
	public void run() {
		...
		while (true) {
			...
			for (int i = 0; i < IBRAChats.size(); i++) {
				InetAddress chkAddress = IBRAChats.get(i).getAddress();
				int chkPort = IBRAChats.get(i).getPort();
				// System.out.println("chk: " + chkPort + " send: " +
				// senderPort);
			...
			if (chat_exists == true) {
				...
				IBRAChats.get(index).msgRec(incMessage);

			} else if (incMessage.substring(0, USER_NAME.length() + 6).equals("????? " + USER_NAME)) {

				// handles broadcasts directed at me
				// LINE TO GET THE USERNAME THAT'S COMMUNICATING WITH ME
				// Review
				...
				// i may have to review this part of my code
	...
	// ### Brodcast protocol Method to start chat session ###
	...
	// BRODCAST protocol when i receive a brodcast
